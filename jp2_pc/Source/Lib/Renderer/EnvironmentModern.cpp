...
bool LoadCubemap(const char* folder){
    const char* names[6] = {"posx.png","negx.png","posy.png","negy.png","posz.png","negz.png"};
    ...
    for(int i=0;i<6;i++){
        ...
        if(!data){
#ifdef __ANDROID__
            __android_log_print(ANDROID_LOG_ERROR, "Trespasser", "Failed to load %s", path.c_str());
#else
...
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO, "Trespasser", "Modern environment init");
#endif
